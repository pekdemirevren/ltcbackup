...
export function RedSpeedSettingsScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const storedSpeed = await AsyncStorage.getItem('redSpeed');
        setRedSpeed(storedSpeed ? parseFloat(storedSpeed) : 1.0);
      } catch (e) {
  ...
  useEffect(() => {
    if (onSave) {
      onSave({ ...settings, redSpeed });
    }
  }, [redSpeed, onSave, settings]);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        >
          ...
          <View style={[styles.valueContainer, { backgroundColor: colors.valueBackground }]}>
            <Text style={styles.valueText}>{Number(redSpeed).toFixed(1)}s</Text>
          </View>
        </TouchableOpacity>
