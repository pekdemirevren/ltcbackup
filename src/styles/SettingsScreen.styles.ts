...
export const getStyles = (colors: ThemeContextType['colors']) => StyleSheet.create({
  ...
  scrollContent: {
    ...
    paddingBottom: 150, // To avoid being hidden behind bottom action/nav bar
  },
  card: {
    borderRadius: 24,
  ...
  cardValue: {
    fontSize: 17,
    color: '#8E8E93', // iOS's standard subdued text color
    marginRight: 8,
  },
  playIconContainer: {
    width: 44,
  ...
  switch: {
    transform: [{ scaleX: 0.9 }, { scaleY: 0.9 }] // To slightly reduce the switch size
  },
  bottomAction: {
    position: 'absolute',
    bottom: 0,
  ...
  // Picker styles (if used directly here, otherwise remove)
