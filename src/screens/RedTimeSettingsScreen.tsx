...
type RedTimeSettingsScreenProps = StackScreenProps<RootStackParamList, 'RedTimeSettingsScreen'> & {
  route: {
    params: {
      isAddMode?: boolean;
    }
...
export function RedTimeSettingsScreen({ route, navigation }: RedTimeSettingsScreenProps) {
  ...
  if (!themeContext || !themeContext.colors || !timerContext) {
    throw new Error('RedTimeSettingsScreen must be used within ThemeProvider and TimerProvider');
  }

  const { colors } = themeContext;
  const { startTimerWithCurrentSettings, setRedTime: setGlobalRedTime } = timerContext;

  const [redTime, setRedTime] = useState(settings?.restTime?.toString() || '30'); // in seconds

  const [isRedPickerVisible, setIsRedPickerVisible] = useState(false);
  const redPickerHeight = useRef(new Animated.Value(0)).current;

  const styles = useMemo(() => getCardButtonPickerStyles(isRedPickerVisible), [isRedPickerVisible]); // Updated to use the new signature
    ...
    const loadSettings = async () => {
      try {
        ...
        if (workoutId) {
          const { loadWorkoutSettings } = await import('../utils/WorkoutSettingsManager');
          const loadedSettings = await loadWorkoutSettings(workoutId);
          setRedTime(loadedSettings.restTime);
        } else {
          const storedTime = await AsyncStorage.getItem('restTime'); // Should this be restTime too? Let's assume global key is consistent.
          if (storedTime) {
  ...
  // Save settings into storage for persistence
  useEffect(() => {
    if (blockId) return;
    const saveSettings = async () => {
      try {
        if (workoutId) {
          ...
          await AsyncStorage.setItem('restTime', redTime);
        }
      } catch (e) {
  ...
  }, [redTime, setGlobalRedTime, workoutId]);
  ...
  const handleStart = async () => {
    // Save the current redTime settings before starting
    try {
      if (workoutId) {
        const { loadWorkoutSettings, saveWorkoutSettings } = await import('../utils/WorkoutSettingsManager');
        const currentSettings = await loadWorkoutSettings(workoutId);
      ...
      console.error("Failed to save red time before start:", e);
    }
        ...
        <View style={{ position: 'absolute', top: 50, right: 24, zIndex: 10 }}>
          <TouchableOpacity
            ...
            style={{
              ...
              backgroundColor: colors.time.primary,
              elevation: 5,
              shadowColor: "#000",
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <TouchableOpacity
          style={[
            styles.card,
            {
              ...
              paddingVertical: 16,
              borderRadius: 24,
              borderBottomLeftRadius: isRedPickerVisible ? 0 : 24,
              borderBottomRightRadius: isRedPickerVisible ? 0 : 24
            }
          ]}
        ...
        <Animated.View style={[styles.pickerWrapper, { height: redPickerHeight, backgroundColor: 'transparent', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', borderTopWidth: 0, marginTop: -1, marginBottom: isRedPickerVisible ? 12 : 0 }]}>
          ...
          <Picker
            ...
            onValueChange={(itemValue: string) => handleRedTimeChange(itemValue)}
