...
const METRIC_COLOR = MetricColors.speed;

export default function CadenceTrendScreen({ navigation }: { navigation: any }) {
  ...
  const [goodCadenceDays, setGoodCadenceDays] = useState(0);
  const [last90Days, setLast90Days] = useState({ good: 0, total: 90 });
  ...
  const loadData = async () => {
    try {
      ...
      // Monthly cadence: average seconds per rep
      ...
      yearFiltered.forEach((s: any) => {
        ...
        if (reps > 0 && duration > 0) {
          const cadence = duration / reps; // seconds per rep
          monthly[month] += cadence;
          monthlyCount[month]++;
        }
      });
      
      for (let m = 0; m < 12; m++) {
        monthly[m] = monthlyCount[m] > 0 ? Math.round(monthly[m] / monthlyCount[m] * 10) / 10 : 0;
      }
      ...
      for (let i = 0; i < 7; i++) {
        ...
        daySummaries.forEach((s: any) => {
          ...
          const duration = s.totalDuration || 0;
          if (reps > 0 && duration > 0) {
            weekly[i] += duration / reps;
            weeklyCount[i]++;
          }
        });
        weekly[i] = weeklyCount[i] > 0 ? Math.round(weekly[i] / weeklyCount[i] * 10) / 10 : 0;
      }
      setWeeklyData(weekly);

      const validWeekly = weekly.filter(v => v > 0);
      const weeklyAvg = validWeekly.length > 0 ? validWeekly.reduce((a, b) => a + b, 0) / validWeekly.length : 0;
      setAverageMetric(Math.round(weeklyAvg * 10) / 10);

      const validMonthly = monthly.filter(v => v > 0);
      setYearlyAverage(validMonthly.length > 0 ? Math.round(validMonthly.reduce((a, b) => a + b, 0) / validMonthly.length * 10) / 10 : 0);

      const currentMonth = now.getMonth();
      const prevMonth = currentMonth === 0 ? 11 : currentMonth - 1;
      // Lower cadence (faster) is better, so inverse trend
      setTrendDirection(monthly[currentMonth] <= monthly[prevMonth] || monthly[prevMonth] === 0 ? 'up' : 'down');

      setTotalWorkouts(yearFiltered.length);

      // Count workouts with good cadence (2-5 seconds per rep is optimal)
      const goodCadenceSet: Set<string> = new Set();
      allSummaries.forEach((s: any) => {
        ...
        if (d.getFullYear() === now.getFullYear()) {
          ...
          const duration = s.totalDuration || 0;
          if (reps > 0 && duration > 0) {
            const cadence = duration / reps;
            if (cadence >= 2 && cadence <= 5) {
              goodCadenceSet.add(d.toDateString());
            }
          }
        }
      });
      setGoodCadenceDays(goodCadenceSet.size);

      // Last 90 days
      const ninetyDaysAgo = new Date(); ninetyDaysAgo.setDate(now.getDate() - 90);
      const last90Good: Set<string> = new Set();
      allSummaries.forEach((s: any) => {
        const d = new Date(s.date);
        if (d >= ninetyDaysAgo && d <= now) {
          ...
          if (reps > 0 && duration > 0) {
            const cadence = duration / reps;
            if (cadence >= 2 && cadence <= 5) {
              last90Good.add(d.toDateString());
            }
          }
        }
      });
      setLast90Days({ good: last90Good.size, total: 90 });
  ...
  const renderYearlyChart = () => {
    ...
    return (
      <View style={styles.yearlyChartContainer}>
        <Text style={styles.chartRightLabel}>SECONDS/REP</Text>
        <View style={styles.chartWithLine}>
          ...
          <View style={[styles.averageLine, { bottom: chartHeight + 20 - ((yearlyAverage / maxVal) * chartHeight) }]}>
            <Text style={styles.averageLineLabel}>{yearlyAverage}s</Text>
          </View>
          <View style={[styles.currentLine, { bottom: chartHeight + 20 - ((averageMetric / maxVal) * chartHeight) }]}>
            <View style={styles.currentLabelBubble}><Text style={styles.currentLineLabel}>{averageMetric}s</Text></View>
          </View>
    ...
    return (
      <View style={styles.weeklyChartContainer}>
        <View style={styles.weeklyChartHeader}>
          ...
          <Text style={styles.weeklyValue}>{averageMetric}s</Text>
        </View>
        <View style={styles.weeklyChartWithLines}>
          ...
          <View style={[styles.weeklyLineLabel, { position: 'absolute', right: 0, top: 0 }]}><Text style={styles.weeklyLineLabelText}>{maxVal}s</Text></View>
          <View style={[styles.weeklyLineLabel, { position: 'absolute', right: 0, top: chartHeight * 0.5 }]}><Text style={styles.weeklyLineLabelText}>{Math.round(maxVal / 2)}s</Text></View>
          <View style={[styles.weeklyLineLabel, { position: 'absolute', right: 0, bottom: 16 }]}><Text style={styles.weeklyLineLabelText}>0s</Text></View>
  ...
  const dayOfYear = Math.floor((new Date().getTime() - startOfYear.getTime()) / (1000 * 60 * 60 * 24)) + 1;
  const yearlyPercent = dayOfYear > 0 ? Math.round((goodCadenceDays / dayOfYear) * 100) : 0;
  const last90Percent = last90Days.total > 0 ? Math.round((last90Days.good / last90Days.total) * 100) : 0;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.mainTitle}>Cadence</Text>
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryText}>
            <Text style={[styles.summaryValue, { color: METRIC_COLOR }]}>{averageMetric}<Text style={styles.summaryUnit}>S/REP</Text></Text>
            <Text style={styles.summaryDescription}>{trendDirection === 'down' ? `Your rep speed may be too fast or too slow. Aim for 2-5 seconds per rep.` : `Great cadence! You're maintaining optimal rep timing.`}</Text>
          </View>
        </View>
        ...
        <View style={styles.ringsSection}>
          <Text style={styles.ringsTitle}>Optimal Cadence Days</Text>
          <Text style={styles.ringsYearly}>{goodCadenceDays}/{dayOfYear} days (%{yearlyPercent})</Text>
          <Text style={[styles.rings90, { color: METRIC_COLOR }]}>{last90Days.good}/{last90Days.total} days (%{last90Percent})</Text>
        </View>
        <View style={styles.separator} />
        <View style={styles.descriptionSection}>
          <Text style={styles.descriptionText}>Cadence measures your rep speed (seconds per repetition). An optimal cadence of 2-5 seconds per rep maximizes muscle tension and growth potential.{' '}<Text style={styles.learnMore}>Learn more...</Text></Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
