...
const METRIC_COLOR = MetricColors.duration;

export default function EnduranceTrendScreen({ navigation }: { navigation: any }) {
  ...
  const loadData = async () => {
    try {
      ...
      const getDuration = (s: any) => Math.round((s.elapsedTime || 0) / 60); // Minutes
      ...
      yearFiltered.forEach((s: any) => {
        ...
        monthly[month] += getDuration(s);
        uniqueDaysPerMonth[month].add(d.toDateString());
      });

      for (let i = 0; i < 12; i++) {
        if (uniqueDaysPerMonth[i].size > 0) monthly[i] = Math.round(monthly[i] / uniqueDaysPerMonth[i].size);
      }
      ...
      for (let i = 0; i < 7; i++) {
        ...
        let total = 0;
        daySummaries.forEach((s: any) => { total += getDuration(s); });
        weekly[i] = Math.round(total);
      }
      setWeeklyData(weekly);

      const validWeekly = weekly.filter(v => v > 0);
      setAverageMetric(validWeekly.length > 0 ? Math.round(validWeekly.reduce((a, b) => a + b, 0) / validWeekly.length) : 0);
      ...
      setTotalDays(Math.floor((now.getTime() - startOfYear.getTime()) / (1000 * 60 * 60 * 24)) + 1);
      ...
      allSummaries.forEach((s: any) => {
        ...
        if (d.getFullYear() === now.getFullYear()) {
          ...
          dailyTotals[key] += getDuration(s);
        }
      });
      
      let activeDaysCount = 0;
      Object.values(dailyTotals).forEach(dur => { if (dur >= dailyGoal) activeDaysCount++; });
      ...
      allSummaries.forEach((s: any) => {
        ...
        if (d >= ninetyDaysAgo && d <= now) {
          ...
          last90Totals[key] += getDuration(s);
        }
      });
      
      let last90ActiveCount = 0;
      Object.values(last90Totals).forEach(dur => { if (dur >= dailyGoal) last90ActiveCount++; });
  ...
  const renderYearlyChart = () => {
    ...
    return (
      <View style={styles.yearlyChartContainer}>
        <Text style={styles.chartRightLabel}>MINUTES</Text>
    ...
    return (
      <View style={styles.weeklyChartContainer}>
        <View style={styles.weeklyChartHeader}>
          ...
          <Text style={styles.weeklyValue}>{Math.max(...weeklyData)} MIN</Text>
        </View>
        <View style={styles.weeklyChartWithLines}>
          ...
          <View style={[styles.weeklyLineLabel, { position: 'absolute', right: 0, top: 0 }]}><Text style={styles.weeklyLineLabelText}>{Math.max(...weeklyData)}</Text></View>
          <View style={[styles.weeklyLineLabel, { position: 'absolute', right: 0, top: chartHeight * 0.5 }]}><Text style={styles.weeklyLineLabelText}>{Math.round(Math.max(...weeklyData) / 2)}</Text></View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.mainTitle}>Endurance</Text>
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryText}>
            <Text style={[styles.summaryValue, { color: METRIC_COLOR }]}>{averageMetric}<Text style={styles.summaryUnit}>MIN/DAY</Text></Text>
            <Text style={styles.summaryDescription}>{trendDirection === 'down' ? `Try to increase your workout duration for better endurance.` : `Great job! Your workout duration is improving.`}</Text>
          </View>
        </View>
        ...
        <View style={styles.ringsSection}>
          <Text style={styles.ringsTitle}>Endurance Goal Reached</Text>
          <Text style={styles.ringsYearly}>{activeDays}/{totalDays} days (%{yearlyPercent})</Text>
          <Text style={[styles.rings90, { color: METRIC_COLOR }]}>{last90Days.active}/{last90Days.total} days (%{last90Percent})</Text>
        </View>
        <View style={styles.separator} />
        <View style={styles.descriptionSection}>
          <Text style={styles.descriptionText}>Endurance measures the total duration of your workouts. Longer workout sessions build cardiovascular endurance and stamina over time.{' '}<Text style={styles.learnMore}>Learn more...</Text></Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
