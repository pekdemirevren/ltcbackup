...
export default function StrengthTrendScreen({ navigation }: { navigation: any }) {
  ...
  const loadData = async () => {
    try {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        let total = 0;
        daySummaries.forEach((s: any) => { total += getVolume(s); });
      ...
      allSummaries.forEach((s: any) => {
        ...
        if (d.getFullYear() === now.getFullYear()) {
          ...
          dailyTotals[key] += getVolume(s);
        }
      });
      
      let activeDaysCount = 0;
      Object.values(dailyTotals).forEach(vol => { if (vol >= dailyGoal) activeDaysCount++; });
      ...
      allSummaries.forEach((s: any) => {
        ...
        if (d >= ninetyDaysAgo && d <= now) {
          ...
          last90Totals[key] += getVolume(s);
        }
      });
      
      let last90ActiveCount = 0;
      Object.values(last90Totals).forEach(vol => { if (vol >= dailyGoal) last90ActiveCount++; });
  ...
  const renderYearlyChart = () => {
    ...
    return (
      <View style={styles.yearlyChartContainer}>
        <Text style={styles.chartRightLabel}>KILOGRAMS</Text>
    ...
    return (
      <View style={styles.weeklyChartContainer}>
        <View style={styles.weeklyChartHeader}>
          ...
          <Text style={styles.weeklyValue}>{Math.max(...weeklyData)} KG</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.mainTitle}>Strength</Text>
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryText}>
            <Text style={[styles.summaryValue, { color: METRIC_COLOR }]}>{averageMetric}<Text style={styles.summaryUnit}>KG/DAY</Text></Text>
            <Text style={styles.summaryDescription}>{trendDirection === 'down' ? `Try to lift more volume to improve your strength gains.` : `Great job! Your training volume is on track.`}</Text>
          </View>
        </View>
        ...
        <View style={styles.ringsSection}>
          <Text style={styles.ringsTitle}>Volume Goal Reached</Text>
        ...
        <View style={styles.descriptionSection}>
          <Text style={styles.descriptionText}>Strength represents your total training volume (weight × sets × reps). Tracking volume helps ensure progressive overload for muscle and strength gains.{' '}<Text style={styles.learnMore}>Learn more...</Text></Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
