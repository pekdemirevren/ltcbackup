...
export function CreateWorkoutBlockScreen({ route, navigation }: CreateWorkoutBlockScreenProps) {
    ...
    const handleStart = () => {
        if (type === 'loop') {
            ...
            // For non-loop types, update relevant context values
            if (type === 'time') {
            ...
            // 'weight' doesn't need context update for starting the timer usually, 
                    ...
                    <>
                        <TouchableOpacity style={[styles.card, { backgroundColor: 'transparent', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', borderRadius: 24 }]} onPress={() => togglePicker(picker1Height, isPicker1Visible, setIsPicker1Visible)} activeOpacity={0.8}>
                            ...
                            <View style={styles.iconContainer}>
                                <Theme.Icons.weight.lib
                                    name={Theme.Icons.weight.name}
                                    size={32}
                                    color={colors.primary}
                                />
                            </View>
                            <Text style={styles.cardTitle}>Weight (kg)</Text>
                            <View style={[styles.valueContainer, { backgroundColor: colors.valueBackground, borderRadius: 12 }]}><Text style={styles.valueText}>{weight}kg</Text></View>
                        </TouchableOpacity>
                        <Animated.View style={{ height: picker1Height, overflow: 'hidden' }}>
