...
type AllWeightMetricsScreenProps = StackScreenProps<RootStackParamList, 'AllWeightMetrics'>;

const { width: SCREEN_WIDTH } = Dimensions.get('window');

type TimeFrame = 'D' | 'W' | 'M' | 'Y';

export default function AllWeightMetricsScreen({ navigation }: AllWeightMetricsScreenProps) {
  ...
  const loadData = async () => {
    try {
      ...
      const initializeData = (size: number) => {
        wData = new Array(size).fill(0);
        sData = new Array(size).fill(0);
        rData = new Array(size).fill(0);
      };
      ...
      if (selectedFrame === 'D') {
        ...
        filtered.forEach((s: any) => {
          ...
          const volume = weightVal * sets * reps;
      ...
      // Average Volume (of active days)
      const activeVol = wData.filter(v => v > 0);
      setAverageVolume(activeVol.length > 0 ? Math.round(activeVol.reduce((a, b) => a + b, 0) / activeVol.length) : 0);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* Weight Volume Chart */}
        {renderChart('STRENGTH VOLUME', averageVolume.toLocaleString(), weightData, '#A358DF', ' KG')}

        {/* Sets Chart */}
        {renderChart('TOTAL SETS', totalSets.toLocaleString(), setsData, '#F9104E')}

        {/* Reps Chart */}
        {renderChart('TOTAL REPS', totalReps.toLocaleString(), repsData, '#00B7FE')}

        <View style={styles.descriptionFooter}>
          <Text style={styles.footerText}>
            Track your training volume (Weight × Sets × Reps) to ensure progressive overload.
          </Text>
        </View>
