...
export default function IntensityScreen({ navigation }: IntensityScreenProps) {
        ...
        try {
            ...
            const aggregate = (summaries: any[], index: number, targetData: number[]) => {
                ...
                summaries.forEach((s: any) => {
                    const ratio = getIntensityRatio(s);
                    if (ratio > 0) {
                        total += ratio;
                        count++;
                    }
                });
                targetData[index] = count > 0 ? parseFloat((total / count).toFixed(2)) : 0;
            };
            ...
            setAverageMetric(avg > 0 ? `${parseFloat(avg.toFixed(1))}:1` : "0:1");
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.statContainer}>
                    <Text style={styles.statLabel}>AVG REST-TO-WORK RATIO</Text>
                    <Text style={[styles.statValue, { color: MetricColors.energy }]}>{averageMetric}</Text>
                    <Text style={styles.statSub}>Rest time relative to active set time</Text>
                </View>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate('AllIntensityMetrics')}
                >
                    <Text style={styles.viewAllText}>View All Intensity Metrics</Text>
                </TouchableOpacity>

                <View style={styles.descriptionFooter}>
                    <Text style={styles.footerText}>
                        Rest-to-Work ratio evaluates the intensity of your training session by comparing your recovery periods to your active work time. A lower ratio typically indicates higher intensity.
                    </Text>
                </View>
            </ScrollView>
        </View>
