...
const METRIC_COLOR = '#00C7BE';

export default function ConsistencyTrendScreen({ navigation }: { navigation: any }) {
  ...
  const loadData = async () => {
    try {
      ...
      // Monthly consistency: percentage of days with workouts
      ...
      for (let m = 0; m < 12; m++) {
        ...
        const uniqueDays = new Set(monthSummaries.map((s: any) => new Date(s.date).toDateString()));
        monthly[m] = Math.round((uniqueDays.size / daysInMonth) * 100);
      }
      setMonthlyData(monthly);

      // Weekly consistency: 1 = worked out, 0 = didn't
      ...
      for (let i = 0; i < 7; i++) {
        ...
        weekly[i] = daySummaries.length > 0 ? 100 : 0;
      }
      setWeeklyData(weekly);

      const activeDaysInWeek = weekly.filter(v => v > 0).length;
      setAverageMetric(Math.round((activeDaysInWeek / 7) * 100));
      ...
      const dailyTotals: Set<string> = new Set();
      allSummaries.forEach((s: any) => {
        const d = new Date(s.date);
        if (d.getFullYear() === now.getFullYear()) {
          dailyTotals.add(d.toDateString());
        }
      });
      setActiveDays(dailyTotals.size);

      const ninetyDaysAgo = new Date(); ninetyDaysAgo.setDate(now.getDate() - 90);
      const last90Set: Set<string> = new Set();
      allSummaries.forEach((s: any) => {
        const d = new Date(s.date);
        if (d >= ninetyDaysAgo && d <= now) {
          last90Set.add(d.toDateString());
        }
      });
      setLast90Days({ active: last90Set.size, total: 90 });
  ...
  const renderYearlyChart = () => {
    ...
    return (
      <View style={styles.yearlyChartContainer}>
        <Text style={styles.chartRightLabel}>PERCENTAGE</Text>
    ...
    return (
      <View style={styles.weeklyChartContainer}>
        <View style={styles.weeklyChartHeader}>
          <Text style={styles.weeklyTitle}>Daily Activity</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.mainTitle}>Consistency</Text>
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryText}>
            ...
            <Text style={styles.summaryDescription}>{trendDirection === 'down' ? `Try to work out more regularly to improve your consistency.` : `Great job! You're maintaining a consistent workout routine.`}</Text>
          </View>
        </View>
        ...
        <View style={styles.ringsSection}>
          <Text style={styles.ringsTitle}>Active Days</Text>
        ...
        <View style={styles.descriptionSection}>
          <Text style={styles.descriptionText}>Consistency measures how regularly you work out. A higher percentage indicates better workout discipline and training adherence.{' '}<Text style={styles.learnMore}>Learn more...</Text></Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
