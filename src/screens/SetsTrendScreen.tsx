...
const METRIC_COLOR = MetricColors.sets;

export default function SetsTrendScreen({ navigation }: { navigation: any }) {
  ...
  const loadData = async () => {
    try {
      ...
      const getSets = (s: any) => s.completedSets || 0;
      ...
      allSummaries.forEach((s: any) => {
        ...
        if (d.getFullYear() === now.getFullYear()) {
          ...
          dailyTotals[key] += getSets(s);
        }
      });
      
      let activeDaysCount = 0;
      Object.values(dailyTotals).forEach(sets => {
        if (sets >= dailyGoal) activeDaysCount++;
      });
      ...
      allSummaries.forEach((s: any) => {
        ...
        if (d >= ninetyDaysAgo && d <= now) {
          ...
          last90Totals[key] += getSets(s);
        }
      });
      
      let last90ActiveCount = 0;
      Object.values(last90Totals).forEach(sets => {
        if (sets >= dailyGoal) last90ActiveCount++;
      });
  ...
  const renderYearlyChart = () => {
    ...
    return (
      <View style={styles.yearlyChartContainer}>
        <Text style={styles.chartRightLabel}>SETS</Text>
                  ...
                  <View 
                    style={[
                      ...
                      { 
                        ...
                        backgroundColor: isCurrentOrRecent ? METRIC_COLOR : '#8E8E93' 
                      }
                    ]} 
                  />
    ...
    return (
      <View style={styles.weeklyChartContainer}>
        <View style={styles.weeklyChartHeader}>
          ...
          <Text style={styles.weeklyValue}>{Math.max(...weeklyData)} SETS</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryText}>
            <Text style={[styles.summaryValue, { color: METRIC_COLOR }]}>{averageMetric}<Text style={styles.summaryUnit}>SETS/DAY</Text></Text>
            <Text style={styles.summaryDescription}>
              {trendDirection === 'down' 
                ? `Try to complete at least ${averageMetric + 5} sets a day to improve your training volume.`
                : `Great job! You're maintaining consistent training volume.`
              }
            </Text>
          </View>
        </View>
        ...
        <View style={styles.ringsSection}>
          <Text style={styles.ringsTitle}>Sets Goal Reached</Text>
        ...
        <View style={styles.descriptionSection}>
          <Text style={styles.descriptionText}>
            Sets represent the number of exercise sets you complete during workouts. Tracking sets helps ensure consistent training volume for muscle growth and strength gains.{' '}
