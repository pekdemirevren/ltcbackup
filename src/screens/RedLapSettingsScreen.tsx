...
type RedLapSettingsScreenProps = StackScreenProps<RootStackParamList, 'RedLapSettingsScreen'> & {
  route: {
    params: {
      isAddMode?: boolean;
    }
...
export function RedLapSettingsScreen({ route, navigation }: RedLapSettingsScreenProps) {
  ...
  if (!themeContext) {
    throw new Error('RedLapSettingsScreen must be used within a ThemeProvider');
  }

  if (!timerContext) {
    throw new Error('RedLapSettingsScreen must be used within a TimerProvider');
  }

  const { colors } = themeContext;
  const { startTimerWithCurrentSettings, setRedRepsWithStorage } = timerContext;
    ...
    const loadSettings = async () => {
      try {
        ...
        if (workoutId && workoutId !== 'default') {
          const { loadWorkoutSettings } = await import('../utils/WorkoutSettingsManager');
          const loadedSettings = await loadWorkoutSettings(workoutId);
          setRedLaps(loadedSettings.redReps);
        } else {
  ...
  const handleStart = async () => {
    try {
      ...
      console.error("Failed to save red laps before start:", e);
    }
