...
type DensityScreenProps = StackScreenProps<RootStackParamList, 'Density'>;

const { width: SCREEN_WIDTH } = Dimensions.get('window');

type TimeFrame = 'D' | 'W' | 'M' | 'Y';

export default function DensityScreen({ navigation }: DensityScreenProps) {
    ...
    const loadData = async () => {
        try {
            ...
            const aggregate = (summaries: any[], index: number, targetData: number[]) => {
                ...
                targetData[index] = count > 0 ? Math.round(total / count) : 0;
            };
            ...
            setAverageMetric(valid.length > 0 ? Math.round(valid.reduce((a, b) => a + b, 0) / valid.length) : 0);
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.statContainer}>
                    <Text style={styles.statLabel}>AVG EXERCISE DENSITY</Text>
                    <Text style={[styles.statValue, { color: '#9DEC2C' }]}>{averageMetric}%</Text>
                    <Text style={styles.statSub}>Active time vs. total workout time</Text>
                </View>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate('AllDensityMetrics')}
                >
                    <Text style={styles.viewAllText}>View All Density Metrics</Text>
                </TouchableOpacity>

                <View style={styles.descriptionFooter}>
                    <Text style={styles.footerText}>
                        Exercise Density measures the quality of your workout by calculating the percentage of time you were actively training versus the total time spent in the session.
                    </Text>
                </View>
            </ScrollView>
        </View>
