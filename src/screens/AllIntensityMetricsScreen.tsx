...
export default function AllIntensityMetricsScreen({ navigation }: AllIntensityMetricsScreenProps) {
    const [selectedFrame, setSelectedFrame] = useState<TimeFrame>('D');
    const [restTimeData, setRestTimeData] = useState<number[]>([]);
    ...
    const loadData = async () => {
        try {
            ...
            setRestTimeData(rData);
            setActiveTimeData(aData);

            const activeDays = aData.filter(v => v > 0).length || 1;
            setAvgRestTime(Math.round(rData.reduce((a, b) => a + b, 0) / activeDays));
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Intensity Metrics</Text>
                <View style={{ width: 28 }} />
            </View>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                {renderChart('REST TIME', Math.round(avgRestTime / 60).toString(), restTimeData, MetricColors.energy, ' MIN')}
                {renderChart('ACTIVE TIME', Math.round(avgActiveTime / 60).toString(), activeTimeData, MetricColors.energy, ' MIN')}
