...
const METRIC_COLOR = '#9DEC2C';

export default function DensityTrendScreen({ navigation }: { navigation: any }) {
  ...
  const [denseDays, setDenseDays] = useState(0);
  const [last90Days, setLast90Days] = useState({ dense: 0, total: 90 });
  ...
  const loadData = async () => {
    try {
      ...
      // Monthly density: work done per minute (sets * reps / duration)
      ...
      yearFiltered.forEach((s: any) => {
        ...
        if (duration > 0) {
          const density = (sets * reps) / (duration / 60); // reps per minute
          monthly[month] += density;
          monthlyCount[month]++;
        }
      });
      ...
      for (let i = 0; i < 7; i++) {
        ...
        daySummaries.forEach((s: any) => {
          ...
          const duration = s.totalDuration || 0;
          if (duration > 0) {
            weekly[i] += (sets * reps) / (duration / 60);
            weeklyCount[i]++;
          }
        });
      ...
      // Count days with high density (> 5 reps per minute)
      const denseDaysSet: Set<string> = new Set();
      const dayDensity: { [key: string]: { total: number, count: number } } = {};
      allSummaries.forEach((s: any) => {
        ...
        if (d.getFullYear() === now.getFullYear()) {
          ...
          const duration = s.totalDuration || 0;
          if (duration > 0) {
            if (!dayDensity[dateStr]) dayDensity[dateStr] = { total: 0, count: 0 };
            dayDensity[dateStr].total += (sets * reps) / (duration / 60);
            dayDensity[dateStr].count++;
          }
        }
      });
      Object.entries(dayDensity).forEach(([date, data]) => {
        const avgDensity = data.total / data.count;
        if (avgDensity >= 5) denseDaysSet.add(date);
      });
      setDenseDays(denseDaysSet.size);

      // Last 90 days
      const ninetyDaysAgo = new Date(); ninetyDaysAgo.setDate(now.getDate() - 90);
      const last90Density: { [key: string]: { total: number, count: number } } = {};
      allSummaries.forEach((s: any) => {
        const d = new Date(s.date);
        if (d >= ninetyDaysAgo && d <= now) {
          ...
          const duration = s.totalDuration || 0;
          if (duration > 0) {
            if (!last90Density[dateStr]) last90Density[dateStr] = { total: 0, count: 0 };
            last90Density[dateStr].total += (sets * reps) / (duration / 60);
            last90Density[dateStr].count++;
          }
        }
      });
      let last90Dense = 0;
      Object.values(last90Density).forEach(data => {
        const avgDensity = data.total / data.count;
        if (avgDensity >= 5) last90Dense++;
      });
      setLast90Days({ dense: last90Dense, total: 90 });
  ...
  const renderYearlyChart = () => {
    ...
    return (
      <View style={styles.yearlyChartContainer}>
        <Text style={styles.chartRightLabel}>REPS/MIN</Text>
    ...
    return (
      <View style={styles.weeklyChartContainer}>
        <View style={styles.weeklyChartHeader}>
          ...
          <Text style={styles.weeklyValue}>{averageMetric} reps/min</Text>
        </View>
        <View style={styles.weeklyChartWithLines}>
          ...
          <View style={[styles.weeklyLineLabel, { position: 'absolute', right: 0, top: 0 }]}><Text style={styles.weeklyLineLabelText}>{Math.round(maxVal)}</Text></View>
          <View style={[styles.weeklyLineLabel, { position: 'absolute', right: 0, top: chartHeight * 0.5 }]}><Text style={styles.weeklyLineLabelText}>{Math.round(maxVal / 2)}</Text></View>
  ...
  const yearlyPercent = totalDays > 0 ? Math.round((denseDays / totalDays) * 100) : 0;
  const last90Percent = last90Days.total > 0 ? Math.round((last90Days.dense / last90Days.total) * 100) : 0;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.mainTitle}>Density</Text>
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryText}>
            <Text style={[styles.summaryValue, { color: METRIC_COLOR }]}>{averageMetric}<Text style={styles.summaryUnit}> REPS/MIN</Text></Text>
            <Text style={styles.summaryDescription}>{trendDirection === 'down' ? `Try to reduce rest times between sets to increase workout density.` : `Great density! You're maximizing work in your workout time.`}</Text>
          </View>
        </View>
        ...
        <View style={styles.ringsSection}>
          <Text style={styles.ringsTitle}>High Density Days</Text>
          <Text style={styles.ringsYearly}>{denseDays}/{totalDays} days (%{yearlyPercent})</Text>
          <Text style={[styles.rings90, { color: METRIC_COLOR }]}>{last90Days.dense}/{last90Days.total} days (%{last90Percent})</Text>
        </View>
        <View style={styles.separator} />
        <View style={styles.descriptionSection}>
          <Text style={styles.descriptionText}>Density measures how much work you perform per minute of training. Higher density workouts are more efficient and can improve both strength and cardiovascular fitness.{' '}<Text style={styles.learnMore}>Learn more...</Text></Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
