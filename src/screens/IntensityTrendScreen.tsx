...
const METRIC_COLOR = MetricColors.energy;

export default function IntensityTrendScreen({ navigation }: { navigation: any }) {
  ...
  const [intenseDays, setIntenseDays] = useState(0);
  const [last90Days, setLast90Days] = useState({ intense: 0, total: 90 });
  ...
  const loadData = async () => {
    try {
      ...
      // Monthly intensity: ratio of active time to rest time (higher = more intense)
      // Calculated as: (total reps * avg rep time) / total duration
      ...
      yearFiltered.forEach((s: any) => {
        ...
        if (duration > 0 && sets > 0) {
          // Intensity score based on energy per minute and sets per minute
          const energyPerMin = energy / (duration / 60);
          const setsPerMin = sets / (duration / 60);
          const intensity = (energyPerMin / 10) + (setsPerMin * 5); // Combined score
          monthly[month] += Math.min(intensity, 100);
          monthlyCount[month]++;
        }
      });
      
      for (let m = 0; m < 12; m++) {
        monthly[m] = monthlyCount[m] > 0 ? Math.round(monthly[m] / monthlyCount[m]) : 0;
      }
      ...
      for (let i = 0; i < 7; i++) {
        ...
        daySummaries.forEach((s: any) => {
          ...
          if (duration > 0 && sets > 0) {
            const energyPerMin = energy / (duration / 60);
            const setsPerMin = sets / (duration / 60);
            const intensity = (energyPerMin / 10) + (setsPerMin * 5);
            weekly[i] += Math.min(intensity, 100);
            weeklyCount[i]++;
          }
        });
        weekly[i] = weeklyCount[i] > 0 ? Math.round(weekly[i] / weeklyCount[i]) : 0;
      }
      ...
      setAverageMetric(Math.round(weeklyAvg));
      ...
      // Count days with high intensity (> 50 score)
      const intenseDaysSet: Set<string> = new Set();
      const dayIntensity: { [key: string]: { total: number, count: number } } = {};
      allSummaries.forEach((s: any) => {
        ...
        if (d.getFullYear() === now.getFullYear()) {
          ...
          if (duration > 0 && sets > 0) {
            ...
            const setsPerMin = sets / (duration / 60);
            const intensity = (energyPerMin / 10) + (setsPerMin * 5);
            if (!dayIntensity[dateStr]) dayIntensity[dateStr] = { total: 0, count: 0 };
            dayIntensity[dateStr].total += Math.min(intensity, 100);
            dayIntensity[dateStr].count++;
          }
        }
      });
      Object.entries(dayIntensity).forEach(([date, data]) => {
        const avgIntensity = data.total / data.count;
        if (avgIntensity >= 50) intenseDaysSet.add(date);
      });
      setIntenseDays(intenseDaysSet.size);

      // Last 90 days
      const ninetyDaysAgo = new Date(); ninetyDaysAgo.setDate(now.getDate() - 90);
      const last90Intensity: { [key: string]: { total: number, count: number } } = {};
      allSummaries.forEach((s: any) => {
        const d = new Date(s.date);
        if (d >= ninetyDaysAgo && d <= now) {
          ...
          if (duration > 0 && sets > 0) {
            ...
            const setsPerMin = sets / (duration / 60);
            const intensity = (energyPerMin / 10) + (setsPerMin * 5);
            if (!last90Intensity[dateStr]) last90Intensity[dateStr] = { total: 0, count: 0 };
            last90Intensity[dateStr].total += Math.min(intensity, 100);
            last90Intensity[dateStr].count++;
          }
        }
      });
      let last90Intense = 0;
      Object.values(last90Intensity).forEach(data => {
        const avgIntensity = data.total / data.count;
        if (avgIntensity >= 50) last90Intense++;
      });
      setLast90Days({ intense: last90Intense, total: 90 });
  ...
  const renderYearlyChart = () => {
    ...
    return (
      <View style={styles.yearlyChartContainer}>
        <Text style={styles.chartRightLabel}>INTENSITY SCORE</Text>
  ...
  const yearlyPercent = totalDays > 0 ? Math.round((intenseDays / totalDays) * 100) : 0;
  const last90Percent = last90Days.total > 0 ? Math.round((last90Days.intense / last90Days.total) * 100) : 0;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.mainTitle}>Intensity</Text>
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryText}>
            <Text style={[styles.summaryValue, { color: METRIC_COLOR }]}>{averageMetric}<Text style={styles.summaryUnit}> SCORE</Text></Text>
            <Text style={styles.summaryDescription}>{trendDirection === 'down' ? `Your workout intensity could be higher. Push harder during your sessions!` : `Great intensity! You're pushing hard in your workouts.`}</Text>
          </View>
        </View>
        ...
        <View style={styles.ringsSection}>
          <Text style={styles.ringsTitle}>High Intensity Days</Text>
          <Text style={styles.ringsYearly}>{intenseDays}/{totalDays} days (%{yearlyPercent})</Text>
          <Text style={[styles.rings90, { color: METRIC_COLOR }]}>{last90Days.intense}/{last90Days.total} days (%{last90Percent})</Text>
        </View>
        <View style={styles.separator} />
        <View style={styles.descriptionSection}>
          <Text style={styles.descriptionText}>Intensity measures how hard you push during workouts, combining energy expenditure and work rate. Higher intensity sessions lead to better fitness gains and calorie burn.{' '}<Text style={styles.learnMore}>Learn more...</Text></Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
