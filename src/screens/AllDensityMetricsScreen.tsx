...
export default function AllDensityMetricsScreen({ navigation }: AllDensityMetricsScreenProps) {
    const [selectedFrame, setSelectedFrame] = useState<TimeFrame>('D');
    const [activeTimeData, setActiveTimeData] = useState<number[]>([]);
    const [totalTimeData, setTotalTimeData] = useState<number[]>([]);
    const [avgActiveTime, setAvgActiveTime] = useState(0);
    const [avgTotalTime, setAvgTotalTime] = useState(0);
    ...
    const loadData = async () => {
        try {
            ...
            if (selectedFrame === 'D') {
                ...
                aData = new Array(7).fill(0);
                tData = new Array(7).fill(0);
                for (let i = 0; i < 7; i++) {
                    const d = new Date();
                ...
                aData = new Array(daysInMonth).fill(0);
                tData = new Array(daysInMonth).fill(0);
                ...
                aData = new Array(12).fill(0);
                tData = new Array(12).fill(0);
            ...
            setActiveTimeData(aData);
            setTotalTimeData(tData);

            const activeDays = tData.filter(v => v > 0).length || 1;
            setAvgActiveTime(Math.round(aData.reduce((a, b) => a + b, 0) / activeDays));
            setAvgTotalTime(Math.round(tData.reduce((a, b) => a + b, 0) / activeDays));
    ...
    const renderChart = (title: string, value: string, data: number[], color: string, unit: string = '') => {
        ...
        const yLabels = [Math.round(maxVal), Math.round(maxVal / 2)];
        const scaleMax = maxVal;
        const CHART_HEIGHT = 150;

        let verticalLines: number[] = [];
        if (selectedFrame === 'W') {
            for (let i = 0; i <= 7; i++) verticalLines.push((i / 7) * 100);
        } else {
                    ...
                    <View style={styles.chartArea}>
                        ...
                        <View style={styles.xAxisContainer}>
                            {getLabels().map((label, i) => {
                                ...
                                if (isWeekly) {
                                    return (
                                        <Text key={i} style={[styles.xAxisLabel, { position: 'absolute', left: `${(i / 7) * 100}%`, marginLeft: 5, bottom: -20, textAlign: 'left' }]}>
                                            {label}
                                        </Text>
                                    );
                                } else if (isGridAligned) {
                                    return (
                                        <Text key={i} style={[styles.xAxisLabel, { position: 'absolute', left: `${i * 25}%`, marginLeft: 5, bottom: -20, textAlign: 'left' }]}>
                                            {label}
                                        </Text>
                                    );
                                } else {
                                    ...
                                    return (
                                        <Text key={i} style={[styles.xAxisLabel, { position: 'absolute', left: `${pct}%`, marginLeft: -15, bottom: -20, textAlign: 'center' }]}>
                                            {label}
                                        </Text>
                                    );
                                }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Density Metrics</Text>
                <View style={{ width: 28 }} />
            </View>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                {renderChart('ACTIVE TIME', Math.round(avgActiveTime / 60).toString(), activeTimeData, '#9DEC2C', ' MIN')}
                {renderChart('TOTAL SESSION TIME', Math.round(avgTotalTime / 60).toString(), totalTimeData, '#9DEC2C', ' MIN')}
...
const styles = StyleSheet.create({
    ...
    selectorContainer: { flexDirection: 'row', backgroundColor: '#1C1C1E', borderRadius: 18, padding: 2, marginBottom: 24 },
    selectorButton: { flex: 1, paddingVertical: 6, alignItems: 'center', borderRadius: 16 },
    selectorButtonActive: { backgroundColor: '#636366' },
    selectorText: { color: '#8E8E93', fontSize: 14, fontWeight: '600' },
    selectorTextActive: { color: '#FFF' },
    chartCard: { backgroundColor: '#1C1C1E', borderRadius: 20, padding: 16, marginBottom: 16 },
    chartHeader: { marginBottom: 16 },
    chartTitle: { fontSize: 13, color: '#8E8E93', fontWeight: '600', marginBottom: 4, textTransform: 'uppercase' },
    chartValue: { fontSize: 24, fontWeight: '500' },
    chartUnit: { fontSize: 16, color: '#8E8E93', fontWeight: '400' },
    chartWrapper: { flexDirection: 'row', height: 180 },
    chartArea: { flex: 1, height: 150, marginRight: 8 },
    gridContainer: { ...StyleSheet.absoluteFillObject, borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#333' },
    gridLine: { position: 'absolute', left: 0, right: 0, height: 1, backgroundColor: '#333' },
    verticalGridLine: { position: 'absolute', top: 0, bottom: 0, width: 1, backgroundColor: '#333' },
    barsContainer: { flex: 1, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' },
    barColumn: { height: '100%', justifyContent: 'flex-end', alignItems: 'center' },
    bar: { width: '60%', minHeight: 4, borderTopLeftRadius: 4, borderTopRightRadius: 4 },
    xAxisContainer: { position: 'absolute', left: 0, right: 0, bottom: 0, height: 20 },
    xAxisLabel: { fontSize: 11, color: '#8E8E93' },
    yAxis: { width: 30, height: 150, justifyContent: 'space-between', marginLeft: 4 },
    axisLabel: { fontSize: 11, color: '#8E8E93', textAlign: 'right', position: 'absolute', right: 0 },
});
